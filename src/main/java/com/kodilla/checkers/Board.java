...
public class Board {
    ...
    public boolean move(int col1, int row1, int col2, int row2) {
        ...
        if(figure.equals(new Pawn(WHITE)) && (row2+col2) % 2 == 0) {
        ...
        } else if(figure.equals(new Pawn(BLACK)) && (row2+col2) % 2 == 0) {
        ...
        } else if((figure.equals(new Queen(WHITE)) || figure.equals(new Queen(BLACK))) && (row2+col2) % 2 == 0) {
...
//tutaj skonczylam dodatkowo sprawdzac
    //dodac kod ktory tworzy krolowe

    public boolean moveWithBeat(int col1, int row1, int col2, int row2, int colEnemy, int rowEnemy) {
        boolean result = false;
        Figure figure = getFigure(col1, row1);
        if(figure.equals(new Pawn(WHITE)) && (row2+col2) % 2 == 0) {
            if(col1 == 0 && row1 >= 0 && row1<7 && col2 == 2 && row2 == row1+2 && row2 < 8 && colEnemy == 1 && rowEnemy == row1+1){
                setFigure(col2, row2, figure);
                setFigure(col1, row1, new None());
                setFigure(colEnemy, rowEnemy, new None());
                result = true;
            } else if(col1 == 7 && row1 >= 0 && row1<7 && col2 == 5 && row2 == row1+2 && row2 < 8 && colEnemy == 6 && rowEnemy == row1+1 && rowEnemy < 8){
                setFigure(col2, row2, figure);
                setFigure(col1, row1, new None());
                setFigure(colEnemy, rowEnemy, new None());
                result = true;
            } else if( col1 > 0 && col1 < 7 && row1 >= 0 && row1<7 && (col2 == col1-1 || col2==col1+1) && row2 == row1+1 && row2 < 8) {
                setFigure(col2, row2, figure);
                setFigure(col1, row1, new None());
                setFigure(colEnemy, rowEnemy, new None());
                result = true;
